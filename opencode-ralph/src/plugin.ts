    ...
    tool: {
      ralph: tool({
        ...
        args: {
          ...
          completionPromise: z.string().optional().describe('Completion signal'),
    ...
    'experimental.chat.system.transform': async (input, output) => {
      if (input.sessionID.includes('ralph')) {
        output.system.push(
          ...
          'You iterate on tasks until completion using the same prompt.',
          'You see previous work through file changes and git history.',
...
function buildRalphPrompt(task: string, options: {
  completionPromise: string;
  options?: {
    verbose?: boolean;
    autoCommit?: boolean;
...
function formatRalphWelcome(): string {
